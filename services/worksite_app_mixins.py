...
class VacancyFilterMixin(object):
    ...
    def filter(
        self,
        params: Dict[str, str],
        company_filter: Optional[User] = None,
        only_not_archived: Optional[bool] = True,
        **kwargs,
    ) -> Dict:
    ...
    def _city_filter(
        self, params: Dict[str, str], company_filter: Optional[User] = None, only_not_archived: Optional[bool] = True
    ) -> Dict:
...
class AddVacancyMixin(DataValidationMixin):
    ...
    def add_vacancy(self, data: Dict, author: User | AnonymousUser) -> DefaultPOSTReturn:
...
class AddOfferMixin(DataValidationMixin):
    ...
    def add_offer(self, applicant: User | AnonymousUser, ids: int, data: Dict, files: Dict) -> DefaultPOSTReturn:
        vacancy = get_object_or_404(Vacancy, pk=ids)
        ...
        offer = Offer(
            applicant=applicant,
            vacancy=vacancy,
            resume=resume_collection.get("resume", None),
    ...
    @staticmethod
    def check_perms(
        applicant: User | AnonymousUser, vacancy: Vacancy, raise_exception: Optional[bool] = True
    ) -> bool | NoReturn:
        if vacancy.archived or vacancy.deleted:
            if raise_exception:
                raise Http404
...
class AddRatingMixin(DataValidationMixin):
    ...
    def add_rating(self, applicant: User | AnonymousUser, uname: str, data: Dict) -> DefaultPOSTReturn | NoReturn:
    ...
    @staticmethod
    def check_perms(applicant: User | AnonymousUser, company: User) -> bool:
...
class ApplyOfferMixin(object):
    ...
    def apply_offer(self, request: Request | HttpRequest, ids: int) -> Offer | NoReturn:
...
class DeleteVacancyMixin(object):
    ...
    def delete_vacancy(self, request: HttpRequest | Request, ids: int) -> Vacancy | NoReturn:
...
class WithdrawOfferMixin(object):
    ...
    def withdraw_offer(self, request: HttpRequest | Request, ids: int) -> Offer | NoReturn:
...
class CompanyApplyedOffersMixin(object):
    ...
    def get_company_applyed_offers(self, company: User | AnonymousUser, check_perms: Optional[bool] = True) -> QuerySet:
        if check_perms:
            CompanyApplyedOffersMixin.check_perms(company)
        return (
            Offer.objects.select_related("applicant", "vacancy")
            .filter(vacancy__company=company, applyed=True)

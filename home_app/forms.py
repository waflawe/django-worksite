...
class AuthForm(forms.Form):
    username = forms.CharField(
        ...
        min_length=5,
        widget=TextInput(
            attrs={"style": "background-color: rgb(20, 20, 20); color: rgb(204, 204, 204)", "class": "form-input"}
        ),
...
class ApplicantRegisterForm(UserCreationForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["username"].help_text = (
            "Длина имени пользователя должна быть 5<=длина<=25, имя не " "должно включать цифры."
        )
        self.fields["password1"].help_text = (
            "Длина пароля должна быть 8<=длина<=64, " "пароль не должен состоять только из цифр."
        )
        ...
        for field, input_ in {"username": TextInput, "password1": PasswordInput, "password2": PasswordInput}.items():
            self.fields[field].widget = input_(
                attrs={"style": "background-color: rgb(20, 20, 20); color: rgb(204, 204, 204)"}
            )
...
class UploadPhotoFormMixin(forms.ModelForm):
    def __init__(self, photo_field: str, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields[photo_field].required, self.fields[photo_field].label = (
            False,
            "Логотип компании:" if photo_field == "company_logo" else "Ваш аватар:",
        )
        self.fields[photo_field].widget = ClearableFileInput(
            attrs={"style": "background-color: rgb(20, 20, 20); color: rgb(204, 204, 204)"}
        )
...
class CompanySettingsForm(UploadPhotoFormMixin):
    company_description = forms.CharField(
        widget=Textarea(attrs={"style": "background-color: rgb(20, 20, 20); color: rgb(204, 204, 204)"}),
        required=False,
        label="Описание компании:",
        validators=[MaxLengthValidator(5000), MinLengthValidator(64)],
    )
    company_site = forms.URLField(
        widget=URLInput(attrs={"style": "background-color: rgb(20, 20, 20); color: rgb(204, 204, 204)"}),
        required=False,
        label="Сайт компании:",
    )
